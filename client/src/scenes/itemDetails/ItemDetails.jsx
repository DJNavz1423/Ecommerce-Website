...
const ItemDetails = () => {
 ...
 async function getItem() {
  ...
  setItem(data.data[0]); // Strapi returns an array for filters
}
  ...
  return (
      ...
      <Box display="flex" flexWrap="wrap" columnGap="40px">
        ...
        <Box flex="1 1 40%" mb="40px">
          <img 
            ...
            width="100%"
            height="100%"
            src={`http://localhost:1337${item?.image?.data?.formats?.medium?.url || item?.image?.url || "/placeholder.png"}`}
    ...
    <p key={i}>
      ...
      <Box display="flex" flexWrap="wrap" gap="15px">
        {value === "description" && (
          <div>{item?.longDescription?.map((block, i) => (
      <p key={i}>
        {block.children?.map((child, j) => (
          <span key={j}>{child.text}</span>
        ))}
